...
public class CreateDeliveryRequirementActivity extends BaseActivity {
    ...
    @Bind(R.id.create_delivery_requirement_package_name)
    EditText mPackageNameExt;
    private String mPackageName;

    @Bind(R.id.create_delivery_requirement_start_location)
    EditText mStartLocationExt;
    private String mStartLocation;

    @Bind(R.id.create_delivery_requirement_end_location)
    EditText mEndLocationExt;
    private String mEndLocation;

    @Bind(R.id.create_delivery_requirement_fragile_cb)
    CheckBox mFragileCb;
    private int mFragile = 0;

    @Bind(R.id.create_delivery_requirement_isbulky)
    CheckBox mBulkyCb;
    private int mBulky = 0;

    @Bind(R.id.create_delivery_requirement_isHeavy)
    CheckBox mHeavyCb;
    private int mHeavy = 0;

    @Bind(R.id.create_delivery_requirement_isinflammable)
    CheckBox mInFlammableCb;
    private int mInflammable = 0;

    @Bind(R.id.create_delivery_requirement_isSample)
    CheckBox mSampleCb;
    private int mSample = 0;

    @Bind(R.id.create_delivery_requirement_description)
    EditText mDescriptionExt;
    private String mDescription;

    @Bind(R.id.create_delivery_requirement_number)
    EditText mNumberExt;
    private int mNumber;

    @Bind(R.id.create_delivery_requirement_weigh)
    EditText mWeighExt;
    ...
    @OnClick(R.id.create_requirement)
    protected void createDelivery() {
        ...
        if (!StringUtils.isEmpty(error)) {
            DialogUtils.showMessageDialog(CreateDeliveryRequirementActivity.this, error);
            return;
        }
        ...
        params.put("ownerName", APIClient.getOwnerAccount(this).getName());

        APIClient.getInstance().execPost("createDeliveryRequirements", params, new Callback() {
            @Override
            public void onFailure(Request request, IOException e) {
                CreateDeliveryRequirementActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        dialog.dismiss();
                    }
            ...
            public void onResponse(Response response) throws IOException {
                ...
                Log.d("Create_BODY", body);
                final ErrorResponse errorResponse = new Gson().fromJson(body, ErrorResponse.class);
                if (!errorResponse.hasError()) {
                    CreateDeliveryRequirementActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            DialogUtils.showMessageDialog(CreateDeliveryRequirementActivity.this, errorResponse.getMessage());
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    CreateDeliveryRequirementActivity.this.finish();
                                }
                            }, TIME_OUT);
                        }
                    });
