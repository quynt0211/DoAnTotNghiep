 ...
 * PRESENA_Android
 * <p/>
 * Created by Paditech on 19/05/2016.
 * Copyright (c) 2015 Paditech. All rights reserved.
 */
public class AccountShipperInSystemAdapter extends BaseAdapter {
    private Context mContext;
    private List<Shipper> mShipper;
    private OnButtonClickListenner onButtonClickListenner;

    public AccountShipperInSystemAdapter(Context context) {
        mContext = context;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.item_account_shipper_in_system, null);
    ...
    public static class ViewHolder {
        @Bind(R.id.item_account_shipper_in_system_shipper_name)
        TextView mShipperName;

        @Bind(R.id.item_account_shipper_in_system_phone)
        TextView mShipperPhone;

        @Bind(R.id.item_account_shipper_in_system_introduce)
        TextView mIntroduce;

        @Bind(R.id.item_account_shipper_in_system_can_delivery)
        TextView mCanDelivery;

        @Bind(R.id.item_account_shippr_in_system_address)
        TextView mAddress;

        @Bind(R.id.item_account_shipper_in_system_block_account)
